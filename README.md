...
M4 version 2 measures excess complexity gradually. Each function, method, or nested
function contributes `CC * sqrt(SLOC)` total mass. Its excess mass is
...
population. Each input has weight 0.5. In `py-2026.2`, erosion's contribution is
...
The default `py-2026.2` calibration profile uses M4 version 2 and the same six
pinned reference projects as `py-2026.1`. Historical profile resources remain
available, but their version 1 erosion distributions cannot score version 2
measurements. Do not directly compare scores across those profiles.
